...
Fix: Open Connect and set the path to ffmpeg.exe. Make sure ffprobe.exe exists in the same folder.
...
Fix: Set the FFmpeg path via Connect. Both ffmpeg.exe and ffprobe.exe must be in the same folder.
...
Built with PowerShell 5.1, WinForms, TMDB REST API, AniList GraphQL API, FFmpeg, and ffprobe.
Designed for Jellyfin media library workflows.
Dark UI theme with custom rounded buttons and menu renderer.
