    ...
    if ($Mode -eq 'TV') {
        if ($TVOptions.Entries) {
            ...
            foreach ($entry in $TVOptions.Entries) {
                ...
                # Season: from filename > fallback box
                $season = if ($det -and $det.Season) { $det.Season } else { $fallbackSeason }

                # Pre-fetch season
                Append-Log "  Fetching Season $season for $($entry.ShowName) from TMDB..." "Gray"
            ...
            foreach ($f in $Files) {
                $det = Get-SeasonEpisodeFromPath $f
                $s   = if ($det -and $det.Season) { $det.Season } else { $fallbackSeason }
...
$panTV.Controls.Add((New-Label "Fallback Season:" 8 68 96))
$txtSeason=New-TextBox 108 66 50 24 "1"; $txtSeason.Add_TextChanged({Update-Preview}); $panTV.Controls.Add($txtSeason)
$panTV.Controls.Add((New-Label "Fallback Ep:" 172 68 78))
$txtStartEp=New-TextBox 252 66 50 24 "1"; $txtStartEp.Add_TextChanged({Update-Preview}); $panTV.Controls.Add($txtStartEp)
$lblFallbackHint=New-Object System.Windows.Forms.Label
$lblFallbackHint.Text="(used only when S##E## not in filename)"
$lblFallbackHint.Location=New-Object System.Drawing.Point(310,70)
$lblFallbackHint.Size=New-Object System.Drawing.Size(220,18)
...
$panTV.Controls.Add((New-Label "Extensions:" 544 68 72))
$txtExts=New-TextBox 618 66 160 24 ".mkv,.mp4,.avi"; $panTV.Controls.Add($txtExts)

$chkShowName=New-Object System.Windows.Forms.CheckBox; $chkShowName.Text="Include show name in filename"
$chkShowName.Location=New-Object System.Drawing.Point(8,92); $chkShowName.Size=New-Object System.Drawing.Size(220,20)
...
$chkTVRenameInPlace.Location=New-Object System.Drawing.Point(236,92); $chkTVRenameInPlace.Size=New-Object System.Drawing.Size(140,20)
...
# ═════════════════════════════════════════════════════════════════════════════
$panFiles=New-Object System.Windows.Forms.Panel; $panFiles.Location=New-Object System.Drawing.Point(12,264)
    ...
    if ($rbTV.Checked) {
        ...
        foreach ($row in $lstFiles.Items) {
            ...
            if ($entry) {
                ...
                # Pre-fetch season (already done above for single show)
            } else {
...
$btnRename.Add_Click({
    ...
    if($rbTV.Checked){
        if ($script:CachedTVEntries) {
            ...
            Start-RenameJob 'TV' ($entries | ForEach-Object { $_.File }) @{Season=$txtSeason.Text.Trim();StartEp=$txtStartEp.Text.Trim();IncludeName=$chkShowName.Checked;Entries=$entries} $null $log $prog $false $renameInPlace
        } else {
            Start-RenameJob 'TV' $script:SelectedFiles @{Season=$txtSeason.Text.Trim();StartEp=$txtStartEp.Text.Trim();ShowName=$script:TVShowData.name;ShowYear=(Get-Year $script:TVShowData.first_air_date);ShowTMDBId=$script:TVShowData.id;IncludeName=$chkShowName.Checked} $null $log $prog $false $renameInPlace
        }
